...
public class EscapeMonster : MonsterBase
{
    ...
    [Header("?룄留? 紐ъ뒪?꽣 information")]
    public bool isEscaping = false; //도망가는 중 변수
    private Vector3 escapeTarget; //도망가는 위치
    [SerializeField]
    private float moveSpeed = 3f; //이동 속도
    [SerializeField]
    private float escapeSpeed = 5f; //도망 속도
    [SerializeField]
    ...
    private void StartEscape(Vector3 targetPosition)
    {
        ...
        nav.speed = escapeSpeed; //도망 속도로 변경
        ...
        {
            //NavMesh에 유효하지 않은 경우, 도망거리를 줄여서 유효한 위치를 찾음
