...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (!DeviceManager.isDeactived || TreasureChest.openingChest || !WeaponChanger.isDeactived || StoreNPCManager.openingStoreUI||Inventory.inventoryActivated)
            {
                if (_hasAnimator)
                {
        ...
        {
            if (!DeviceManager.isDeactived || TreasureChest.openingChest || StoreNPCManager.openingStoreUI ||Inventory.inventoryActivated) return; // 비활성화 상태면 카메라 회전 막기 , 보물상자 오픈시 카메라 회전 막기
            CameraRotation();
        }
